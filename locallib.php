...
/**
 * Theme Boost proitec - Locallib file
 *
...
//  * @return string
//  */
// function theme_boost_proitec_get_random_loginbackgroundimage_class() {
//
//     // Fetch context.
...
//     $files = $fs->get_area_files($systemcontext->id, 'theme_boost_proitec', 'loginbackgroundimage', false, 'itemid', false);
//
...
//  * @return string
//  */
// function theme_boost_proitec_get_loginbackgroundimage_scss() {
//     $count = 0;
//     $scss = "";
//
//     // Fetch context.
...
//  * @return array
//  */
// function theme_boost_proitec_get_imageareacontent() {
//     // Get cache.
//     $themeboostproiteccache = cache::make('theme_boost_proitec', 'imagearea');
//     // If cache is filled, return the cache.
//     $cachecontent = $themeboostproiteccache->get('imageareadata');
...
//         $fs = get_file_storage();
//         // Get all files from filearea.
//         $files = $fs->get_area_files($systemcontext->id, 'theme_boost_proitec', 'imageareaitems', false, 'itemid', false);
//
...
//             $lines = explode("\n", get_config('theme_boost_proitec', 'imageareaitemslink'));
//             // Parse item settings.
...
//                     // Fill the cache.
//                     $themeboostproiteccache->set('imageareadata', $imageareacache);
...
//             return $themeboostproiteccache->set('imageareadata', array());
//         }
//     }
// }
 ...
 */
function theme_boost_proitec_process_flatnav(flat_navigation $flatnav) {
    global $USER;
    // If the setting defaulthomepageontop is enabled.
    if (get_config('theme_boost_proitec', 'defaulthomepageontop') == 'yes') {
        ...
        if (($coursehomenode = $flatnav->find('coursehome', global_navigation::TYPE_CUSTOM)) != false) {
            ...
            if (get_config('core', 'defaulthomepage') == HOMEPAGE_SITE) {
                ...
                $flatnavreturn = theme_boost_proitec_set_node_on_top($flatnav, 'home', $coursehomenode);
            } else if (get_config('core', 'defaulthomepage') == HOMEPAGE_MY) { // If the dashboard is set as the default homepage
                // by the admin.
                // Return the modified flat_navigtation.
                $flatnavreturn = theme_boost_proitec_set_node_on_top($flatnav, 'myhome', $coursehomenode);
 ...
 * @return flat_navigation.
 */
function theme_boost_proitec_set_node_on_top(flat_navigation $flatnav, $nodename, $beforenode) {
 ...
 */
function theme_boost_proitec_get_incourse_settings() {
    global $COURSE, $PAGE;
    // Initialize the node with false to prevent problems on pages that do not have a courseadmin node.
    $node = false;
    // If setting showsettingsincourse is enabled.
    if (get_config('theme_boost_proitec', 'showsettingsincourse') == 'yes') {
        ...
        if ($PAGE->context->contextlevel == CONTEXT_COURSE || $PAGE->context->contextlevel == CONTEXT_MODULE) {
            ...
            if (!empty($node)) {
                ...
                if (get_config('theme_boost_proitec', 'incoursesettingsswitchtorole') == 'yes' && !is_role_switched($COURSE->id)) {
                    ...
                    if (is_array($roles) && (count($roles) > 0)) {
                        ...
                        $properties = array('text' => get_string('switchroleto', 'theme_boost_proitec'),
 ...
 * @return navigation_node.
 */
function theme_boost_proitec_get_incourse_activity_settings() {
    global $PAGE;
